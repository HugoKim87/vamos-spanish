  ...
  <div v-if="tutorial" class="container">
    ...
    <article class="body">
      <section v-for="(s, i) in tutorial.sections" :key="i" class="section">
        ...
        <div v-if="s.alphabet" class="alpha-grid">
          <div v-for="a in s.alphabet" :key="a.letter" class="alpha-card">
            <div class="a-head">
              <span class="a-letter es-text">{{ a.letter }}</span>
              <span class="a-name">{{ a.name }}</span>
              <SpeakButton :text="a.say" size="sm" />
            </div>

            <p v-if="a.note" class="a-note">{{ a.note }}</p>

            <ul class="a-words">
              <li v-for="w in a.words" :key="w.es" class="a-word">
                <SpeakButton :text="w.es" size="sm" />
                <span class="w-es es-text">{{ w.es }}</span>
                <span class="w-read">{{ w.read }}</span>
                <span class="w-ko">{{ w.ko }}</span>
              </li>
            </ul>
          </div>
        </div>
...
.a-head { display: flex; align-items: center; gap: var(--sp-3); }
.a-letter {
  font-size: 20px; font-weight: 900; letter-spacing: -.02em;
  color: var(--c-primary); min-width: 52px;
}
.a-name { font-size: 14px; font-weight: 700; color: var(--c-text-soft); flex: 1; }
.a-note {
  margin-top: var(--sp-2);
...
.a-words { margin-top: var(--sp-3); display: flex; flex-direction: column; gap: 4px; }
.a-word {
  display: flex; align-items: center; gap: var(--sp-2);
  padding: 5px 0;
}
.w-es { font-weight: 700; font-size: 14px; min-width: 88px; }
.w-read { font-size: 12.5px; color: var(--c-primary); min-width: 74px; }
.w-ko { font-size: 12.5px; color: var(--c-text-mute); }
...
@media (max-width: 560px) {
  ...
  .w-es, .w-read { min-width: unset; }
  .a-word { flex-wrap: wrap; gap: 6px; }
}
</style>
