...
- estar가 활용표에 없어 estoy가 아닌 esto로 활용됨 (dar도 동일)
...
| `dictation.test.js` | 받아쓰기 재시도 흐름, 채점 후 입력란이 사라지는지 |

### 테스트 작성 시 주의
- **무작위에 기대지 말 것.** 학습 모드는 카드 섞기·문제 유형 선정에 `Math.random`을 쓴다.
  `tests/setup.js`에서 씨앗값 기반으로 고정해 두었으니, 실행마다 결과가 같다.
  그래도 "특정 유형이 나올 때까지 넘기는" 코드는 큐가 바닥난 경우를 반드시 구분해서
  실패 메시지에 드러나게 한다. (횟수만 세면 `expected undefined to be 'fill'`처럼
  원인을 알 수 없는 실패가 된다)
